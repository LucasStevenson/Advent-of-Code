...
for hand, _ in lines:
    ...
    if "J" in num_each_card:
        jAmt = num_each_card["J"]
        num_each_card.pop("J")
        if len(num_each_card) == 0:
            # JJJJJ
            arr.append((hand, 5))
            continue
        most_occurance_card = max(num_each_card, key=num_each_card.get)
        num_each_card[most_occurance_card] += jAmt
...
def compare(c1, c2):
    ...
    card_to_val = { c: i for i, c in enumerate("J23456789TQKA") }
